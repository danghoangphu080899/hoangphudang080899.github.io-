...
@section('allAmin')
    ...
    background: #ea6153;">
                  ...
                  <tr id="del_ajax{{$all->idNhanVien}}">
                    
                    <td id="{{$all->idNhanVien}}">{{$all->idNhanVien}}</td>
                    <td><b>{{$all->HoTen}}</b></td>
                    @if($all->avatar ==null)
                    ...
                    <td>{{$all->email}}</td> 
                    @if($all->TrangThai == 1)
                    <td class="text-center"><span class="badge badge-danger">Hoạt động</span></td>
                    @else
                    <td class="text-center"><span class="badge badge-secondary">Khóa</span></td>
                    @endif
                    <td class="table-action text-center mr-auto">
                        <a href="" title="Xem thông tin" data-toggle="modal" data-target="#edit_user_{{$all->idNhanVien}}"><i class="align-middle" data-feather="eye"></i></a>
                        
                        @if($all->TrangThai==0)
                        <a title="Mở khóa" href="javascript:void" onclick="show_admin({{$all->id}})"><i class="align-middle" data-feather="unlock"></i></a>
                        @else
                        <a title="Khóa" href="javascript:void" onclick="hidden_admin({{$all->id}})"><i class="align-middle" data-feather="lock"></i></a>
                        @endif
...
<div class="modal fade" id="edit_user_{{$user->idNhanVien}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="col-md-9">
                    ...
                    <div class="form-row col-md-12">
                      ...
                      <div class="form-group col-md-12">
                       ...
                       <textarea disabled="" type="text" class="form-control" >{{$user->DiaChi}}</textarea>
